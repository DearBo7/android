...
public class SwipeMenuLayout extends ViewGroup {
    ...
    /**
     * 20180219add如果当前滑动是禁止的,然后点击禁止的,关闭其他已经滑动的
     */
    private boolean isSwipeClickEnable;
    ...
    public void setSwipeClickEnable(boolean swipeClickEnable) {
        isSwipeClickEnable = swipeClickEnable;
    }
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            } else if (attr == R.styleable.SwipeMenuLayout_swipeClickEnable) {
                isSwipeClickEnable = ta.getBoolean(attr, true);
            }
        }
        ta.recycle();
...
/*                    if (!mScroller.isFinished()) {
            ...
            //isSwipeClickEnable:false 如果当前滑动是禁止的,然后点击禁止的关闭其他已经滑动的.
            if (!isSwipeClickEnable) {
                if (mViewCache != null) {
                    if (mViewCache != this) {
                        mViewCache.smoothClose();
                    }
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        //isSwipeEnable:false禁止侧滑时，点击事件不受干扰。
        if (isSwipeEnable) {
            switch (ev.getAction()) {
