...
public class SwitchButton extends View implements Checkable {
    //58
    private static final int DEFAULT_WIDTH = dp2pxInt(86);
    //36
    ...
    private int btnStartTextColor;
    private int btnEndTextColor;
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        btnStartText = optString(typedArray, R.styleable.SwitchButton_sb_start_text, "");
        btnEndText = optString(typedArray, R.styleable.SwitchButton_sb_end_text, "");
        btnStartTextColor = optColor(typedArray, R.styleable.SwitchButton_sb_start_text_color, Color.WHITE);
        btnEndTextColor = optColor(typedArray, R.styleable.SwitchButton_sb_end_text_color, Color.WHITE);
        btnTextSize = optFloat(typedArray, R.styleable.SwitchButton_sb_text_size, 14);
    ...
    private void drawButtonText(Canvas canvas) {
        //开始文字
        if (!isChecked && btnStartText != null && !"".equals(btnStartText.trim())) {
            float fontWidth = paint.measureText(btnStartText) / 2;
            paint.setColor(btnStartTextColor);
            drawButtonText(canvas, btnStartText, buttonMinX - fontWidth, getHeight(), paint);
        }
        //结束文字
        if (isChecked && btnEndText != null && !"".equals(btnEndText.trim())) {
            float fontWidth = paint.measureText(btnEndText) / 2;
            paint.setColor(btnEndTextColor);
            drawButtonText(canvas, btnEndText, buttonMaxX - fontWidth, getHeight(), paint);
        }
    }

    private void drawButtonText(Canvas canvas, String text, float x, float height, Paint paint) {
        paint.setTextSize(btnTextSize);
        Paint.FontMetrics fontMetrics = paint.getFontMetrics();
        height = height / 2 + (Math.abs(fontMetrics.ascent) - fontMetrics.descent) / 2;
    ...
    private static float optFloat(TypedArray typedArray, int index, float def) {
        if (typedArray == null) {
            return def;
        }
